...
function Orb({ color, size, from, to, duration }: { color: string; size: number; from: [number, number]; to: [number, number]; duration: number }) {
  ...
  return (
    <Animated.View
      pointerEvents="none"
      style={[{ position: 'absolute', width: size, height: size, borderRadius: size / 2, backgroundColor: color, opacity: 0.55 }, blur, animated]}
    />
  );
}
...
export function Background() {
  ...
  return (
    <View style={[StyleSheet.absoluteFill, { overflow: 'hidden' }]} pointerEvents="none">
      ...
      <Orb color={theme.orbs[0]} size={size} from={[-size * 0.3, -size * 0.2]} to={[width * 0.3, height * 0.1]} duration={17_000} />
      <Orb color={theme.orbs[1]} size={size * 0.8} from={[width * 0.6, height * 0.35]} to={[width * 0.1, height * 0.55]} duration={21_000} />
      <Orb color={theme.orbs[2]} size={size * 0.9} from={[width * 0.1, height * 0.8]} to={[width * 0.55, height * 0.6]} duration={25_000} />
    </View>
  );
}
